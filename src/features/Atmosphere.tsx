...
const AREAS = [
  {
    ...
    description:
      "Um canto dedicado ao Cavaleiro das Trevas com colecionáveis, artes e a atmosfera sombria de Gotham City.",
    icon: Zap,
  ...
  },
  {
    ...
    description:
      "Action figures, pôsteres vintage, luminosos neon e toda a cultura pop dos anos 80 e 90 reunida num espaço temático intergaláctico.",
    icon: Gamepad2,
  ...
  },
  {
    ...
    description:
      "Dance ao som dos maiores hits dos anos 80 e 90 comandados pelo nosso DJ ao vivo! Iluminação retrô e muita energia na batida certa.",
    icon: Music,
...
export default function Atmosphere() {
  ...
  const [selectedArea, setSelectedArea] = useState<(typeof AREAS)[0] | null>(
    null,
  );
  ...
  return (
    <>
              ...
              return (
                ...
                >
                    ...
                    <picture>
                      {!area.image.endsWith(".avif") && (
                        <source
                          ...
                          srcSet={area.image.replace(".webp", "-sm.webp")}
                        />
                      )}
                      <img
                        src={area.image}
                    ...
                    {area.hoverImage && (
                      <picture>
                        {!area.hoverImage.endsWith(".avif") && (
                          <source
                            media="(max-width: 768px)"
                            srcSet={area.hoverImage.replace(".webp", "-sm.webp")}
                          />
                        )}
                        <img
                          src={area.hoverImage}
                  ...
                  <div className="relative z-10 p-6 md:p-8 flex flex-col items-center text-center gap-4">
                    ...
                    <div>
                      <h3
                        className={`font-pixel text-sm md:text-base mb-3 ${area.accent}`}
                      >
      ...
      {/* Modal rendered OUTSIDE the section to escape its z-1 stacking context */}
      <AnimatePresence>
        {selectedArea && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 md:p-12">
            ...
            >
              ...
              <div className="w-full md:w-3/5 h-[35vh] md:h-auto min-h-[40vh] relative overflow-hidden rounded-t-lg md:rounded-l-lg md:rounded-tr-none z-10">
                <picture>
                  {!(selectedArea.hoverImage || selectedArea.image).endsWith(
                    ".avif",
                  ) && (
                      <source
                        media="(max-width: 768px)"
                        srcSet={(
                          selectedArea.hoverImage || selectedArea.image
                        ).replace(".webp", "-sm.webp")}
                      />
                    )}
                  <img
              ...
              <div className="w-full md:w-2/5 p-6 md:p-10 flex flex-col justify-center overflow-y-auto">
                <div
                  className={`w-14 h-14 rounded-lg bg-black/50 border-2 border-current/20 flex items-center justify-center mb-6 shadow-lg ${selectedArea.accent}`}
                >
                  <selectedArea.icon className="w-7 h-7" />
                </div>
                <h3
                  className={`font-pixel text-xl md:text-2xl lg:text-3xl mb-6 ${selectedArea.accent} drop-shadow-[0_0_10px_currentColor]`}
                >
