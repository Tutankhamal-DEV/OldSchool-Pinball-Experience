...
export default function Footer() {
    ...
    const bgInView = useInView(ref, { amount: 0.1 })

    useEffect(() => {
        if (bgInView) {
            window.dispatchEvent(new CustomEvent("set-canvas-scene", { detail: 7 })) // 'racing' scene for footer
        }
    }, [bgInView])

    return (
        <footer ref={ref} className="relative border-t border-pinball-red/20 py-8 md:py-12 px-4 bg-transparent md:bg-black/25 z-50">
            <div className="max-w-6xl mx-auto">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-8 text-center sm:text-left">
                    ...
                    <div className="col-span-1 sm:col-span-2 md:col-span-1">
                        ...
                        <p className="font-body text-sm text-pinball-cream/50 leading-relaxed max-w-md mx-auto sm:mx-0">
                    ...
                    <div>
                        ...
                        <nav className="space-y-2">
                            <a
                                href="#ingressos"
                                className="block font-body text-sm text-pinball-cream/40
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div className="text-center sm:text-right flex flex-col items-center sm:items-end gap-2 drop-shadow-md mt-4 sm:mt-0">
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex flex-col items-center sm:items-end font-pixel text-xs md:text-sm text-pinball-red hover:text-pinball-neon-red hover:animate-neon-pulse transition-all duration-300 uppercase tracking-wider mt-2 group"
                        >
                            <span className="opacity-90 group-hover:opacity-100 transition-opacity mb-1 capitalize">
                                {String(t('footer.developer_credit')).split(':')[0]?.toLowerCase()}:
                            </span>
                            <span className="font-tech text-base tracking-widest text-[#ff2a2a] group-hover:text-white transition-colors duration-300 drop-shadow-[0_0_8px_rgba(255,42,42,0.8)]">
                                <span className="glitch-text" data-text="TUTANKHAMAL DEV">TUTANKHAMAL DEV</span>
                            </span>
                        </a>
                    </div>
                </div>
