...
export default function Chatbot() {
  ...
  const nextId = () => (++msgIdRef.current).toString();
  ...
  // Initialize bot welcome message when opened for the first time
  ...
  const handleSendMessage = (text: string) => {
    ...
    const keywordsTickets = t("chatbot.keywords.tickets", {
      defaultValue: "ingressos,preço,valor,tickets,price,buy,cost",
    }).split(",");
    const keywordsLocation = t("chatbot.keywords.location", {
      defaultValue: "onde,local,endereço,where,location,address,map",
    }).split(",");
    const keywordsGames = t("chatbot.keywords.games", {
      defaultValue: "jogos,máquinas,pinball,games,machines,play",
    }).split(",");

    if (keywordsTickets.some((kw) => lowerText.includes(kw.trim())))
      responseKey = "tickets";
    else if (keywordsLocation.some((kw) => lowerText.includes(kw.trim())))
      responseKey = "location";
    else if (keywordsGames.some((kw) => lowerText.includes(kw.trim())))
      responseKey = "games";
      ...
      if (responseKey) {
        setMessages((prev) => [
          ...prev,
          {
            id: nextId(),
            sender: "bot",
            text: t(`chatbot.answers.${responseKey}`),
          },
          {
            id: nextId(),
            sender: "bot",
  ...
  return (
    <>
      {/* Floating CTA Button */}
      <div
        className={`fixed right-6 z-[100] flex flex-col items-end gap-2 transition-all duration-300 ${isAtBottom ? "bottom-40 sm:bottom-48" : "bottom-6"}`}
      >
        <AnimatePresence>
          {!isOpen && showTooltip && (
            <motion.div
              ...
              className="bg-pinball-black border border-pinball-neon-red/50 text-pinball-cream font-pixel text-[10px] sm:text-xs py-2 px-3 rounded shadow-[0_0_10px_rgba(196,30,42,0.6)]"
            >
              {t("chatbot.tooltip")}
            </motion.div>
          )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      <span className="font-pixel text-[8px] text-green-500/80 uppercase">
                        {t("chatbot.status_online")} - v1.0.4
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {messages.map((msg) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        className={`p-3 rounded-lg text-sm font-body leading-relaxed border backdrop-blur-sm ${
                          msg.sender === "user"
              ...
              <div className="p-4 border-t border-pinball-red/30 bg-pinball-black/60">
                ...
                >
                  <input
                    ...
                    placeholder={
                      t("chatbot.placeholder") || "Digite sua mensagem..."
                    }
